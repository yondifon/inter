...
export const DELEGATE_DESCRIPTION = [
  ...
  "Say how hard the work is with difficulty and leave profile and model out — Inter picks the account, the model, and how much it thinks, from that plus what the project allows and how much usage each account has left.",
  "Name a profile or a model when the user asked for one. That choice always wins; the response warns you when the account is out of credits or rate-limited, when it does not offer the model, or when the project does not allow that model for this kind of work — all of which used to pass silently.",
